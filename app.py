...
UUID = os.environ.get('UUID', 'e1dfaa2c-aab2-4e19-b2f1-ccdc7b684f06')       # UUID
NEZHA_SERVER = os.environ.get('NEZHA_SERVER', 'nz.f4i.cn')                  # 哪吒3个变量不全不运行
NEZHA_PORT = os.environ.get('NEZHA_PORT', '8443')                           # 哪吒端口为{443,8443,2096,2097,2083}其中之一时自动开启tls
NEZHA_KEY = os.environ.get('NEZHA_KEY', '')                                 # 哪吒客户端密钥
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', 'boxskk.531957.xyz
')                             # 国定隧道域名，留空即启用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', 'eyJhIjoiMzdhNDU0OTVmMGQ1ZDA4Y2Q5MDgyOWUwMzdhMmZmNjYiLCJ0IjoiNzc0YTNjOGEtY2IwYy00NmJiLWE0ZWEtOTU0YzQ5M2Y1ZjVmIiwicyI6Ik16Vm1aalEzWXpBdFpqVXlNQzAwTWpnekxUbGpaREl0TkRnMFkyRXdabUZoWldFMSJ9')                                 # 国定隧道json或token，留空即启用临时隧道,json获取地址：https://fscarmen.cloudflare.now.cc
ARGO_PORT = int(os.environ.get('ARGO_PORT', 13975))                          # Argo端口，固定隧道token请改回8080或在cf后台设置的端口与这里对应
